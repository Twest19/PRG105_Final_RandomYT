...
class RandomVid:
    ...
    def get_options(self, select, urls, option, urls2):  # Takes users selected options and continues depending on them
        ...
        notice_label = Label(self.bot_frame, bg='gray', wraplength=250)
        ...
        if options != 'Options':
            ...
            if options == 'Open All':
                ...
                notice_label.config(text='All videos have been opened in the default browser.')
                ...
                notice_label.config(text='Click the \"next\" and \"prev\" buttons to change videos.')

            elif options == 'Play Locally':
                self.destroy_default_buttons()
                # Opens the local tkinter window that has VLC media player embedded in it
                notice_label.config(text='Video are playing in a local tkinter with VLC media player embedded. '
